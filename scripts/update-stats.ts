...
// Helper functions for repo stats
	...
	try {
		...
		for (const entry of entries) {
			if (entry.isFile()) {
				...
				count += await countFilesInDirectory(
...
async function generateRepoStats(): Promise<RepoStats> {
	...
	const adminComponents = await countFilesInDirectory(
		join(rootPath, "app/components"),
		/^Admin.*\.vue$/,
	);
	const appComponents = await countFilesInDirectory(
		join(rootPath, "app/components"),
		/^App.*\.vue$/,
	);
	const pages = await countFilesInDirectory(
		join(rootPath, "app/pages"),
		/\.vue$/,
	);
	const layouts = await countFilesInDirectory(
		join(rootPath, "app/layouts"),
		/\.vue$/,
	);
	...
	const songsCount = await countFilesInDirectory(
		join(rootPath, "server/assets/songs"),
		/\.json$/,
	);
	const songsSize = await getDirectorySize(
		join(rootPath, "server/assets/songs"),
	);
	const playlistsCount = await countFilesInDirectory(
		join(rootPath, "public/playlist"),
		/\.json$/,
	);

	// Check for quality indicators
	const hasESLint =
		files.extensions[".mjs"] > 0 || packageJson.devDependencies?.eslint;
	const hasTests =
		files.extensions[".test.ts"] > 0 || files.extensions[".spec.ts"] > 0;
...
async function updateSources(): Promise<void> {
	...
	const repoStats = await generateRepoStats();
	...
	sourcesData.repository = repoStats;
	...
	console.log(`- Total Files: ${repoStats.files.totalFiles}`);
	console.log(`- Total Lines: ${repoStats.files.totalLines}`);
	console.log(`- Total Size: ${formatBytes(repoStats.files.totalSize)}`);
	console.log(`- Vue Components: ${repoStats.components.total}`);
	console.log(`- API Endpoints: ${repoStats.api.total}`);
}
